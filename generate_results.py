...
random_sreads = random_sreads.sample(n=1000)  # Sample n random short reads from total alined short reads.

# Outlier analysis Data-structures
...
shoot = [] # Misaligned reads container
...
for i in random_sreads.index:
    ...
    if random_sreads['ref_start'][i] < 0 or np.isnan(random_sreads['ref_end'][i]):
        ...
        if start_deviation > random_sreads['qlength'][i]:
            temp.append(random_sreads['qlength'][i] + 1)  # Set outlier to a number which is 1 greater than the short read length
            outlier_start_tf.append(random_sreads['read'][i])
            outlier_read_type.append(read_type)
        else:
            temp.append(start_deviation)
            normal_start_tf.append(random_sreads['read'][i])
        
        if end_deviation > random_sreads['qlength'][i]:
            temp.append(random_sreads['qlength'][i] + 1) # Set outlier to a number which is 1 greater than the short read length
            outlier_end_tf.append(random_sreads['read'][i])
        else:
            temp.append(end_deviation)
            normal_end_tf.append(random_sreads['read'][i])
        
        if percentage_overlap > 100:
            temp.append(0) # No overlap
            outlier_overlap_tf.append(random_sreads['read'][i])
        else:
            temp.append(percentage_overlap)
            normal_overlap_tf.append(random_sreads['read'][i])
